...
los ceros están en el circulo unidad y los polos están en 0
...
y1_2 <- as.data.frame(p1)
indice <- c(1:nrow(y1_2))
y1_2 <- cbind(indice, y1_2)
y1_2 <- as.data.frame(y1_2)

ggplot(y1_2) + geom_line(aes(x=indice, y=x)) + xlim(0,2500)
...
y2_2 <- as.data.frame(salida)
indice <- c(1:nrow(y2_2))
y2_2 <- cbind(indice, y2_2)
y2_2 <- as.data.frame(y2_2)
...
El parámetro mu controla los polos

wuolah:
Como mu está en el denominador del segundo filtro, si modificamos mu estaríamos modificando los polos. Además, podemos observar que la respuesta impulsional a medida que mu se acerca a 1 se va atenuando y se aproxima más y más a 0.
