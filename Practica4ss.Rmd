...
y1 <- filter(num, den, ecg)

# Representamos el ECG tras aplicar el primer filtro

plot(y1, xlim=c(0,2500))

bl1 <- as.data.frame(y1) # pasamos a data frame la variable que queremos representar
indice2 = c(1:nrow(bl1)) #creamos un vector con el número de las filas que hay en nuestro df 
bl2 = cbind(indice2,bl1)
ggplot(bl2) + geom_line(aes(x= indice2, y= y1)) + ggtitle("Electrocardiograma") #xlim=c(0,2500)
...
plot(y2, xlim=c(0,2500))

bl1 <- as.data.frame(y2) # pasamos a data frame la variable que queremos representar
indice2 = c(1:nrow(bl1)) #creamos un vector con el número de las filas que hay en nuestro df 
bl2 = cbind(indice2,bl1)
ggplot(bl2) + geom_line(aes(x= indice2, y= y2)) + ggtitle("Electrocardiograma con mu = 0.9") #xlim=c(0,2500)
...
ggplot(bl2) + geom_line(aes(x= indice2, y= y2)) + ggtitle("Electrocardiograma con mu = 0.3") #xlim=c(0,2500)
...
A medida que el parametro $\mu$ se va acercando al 0, larespuesta de la señal esta cada vez mas comprimida y se va acercando mas al 0 como se puede comprobar graicamente con la funcion plot. Ya que en $\mu= 0.9$ como pedia el enunciado, los valores obtenidos en la respuesta rondan entre -0.3 y 0.3, mientras que con $\mu= 0.3$ los valores rondan entre -0.05 y 0.05.

3. ¿Qué controla el parámetro $\mu$?

El parámetro mu controla los polos, puesto que esta en el denominador de la funcion H(z). Además, se puede comprobar graficamete con la funcion zplane porque lo unico que cambia son las cruces(polos).
